...
import HostControl from 'components/Dashboard/HostControl/HostControl';
...
function Dashboard() {
    ...
    const [userAddressLatLng, setUserAddressLatLng] =
        useState<Coordinates | null>(null);
    const [hostDistances, setHostDistances] = useState<HostDistances | null>(
        null,
    );
    ...
    const handleSortHostData = (weights: Weights) => {
        ...
        const sortedData = quickSort(
            [...hostData],
            (host1: Host, host2: Host) => {
                const weightedScore1 = hostWeightedScore(
                    host1,
                    weights,
                    hostDistanceScores,
                );
                const weightedScore2 = hostWeightedScore(
                    host2,
                    weights,
                    hostDistanceScores,
                );
    ...
    useEffect(() => {
        if (userAddressLatLng) {
            ...
            hostData.forEach((host) => {
                let d: number = distance(userAddressLatLng, {
                    lat: host.latitude,
                    lng: host.longitude,
                });
    ...
    return (
        <>
            ...
            >
                <MapDisplay
                    locationMarkerData={userAddressLatLng}
                    markerData={markerData()}
                    offsetX={getMapOffset()}
                >
